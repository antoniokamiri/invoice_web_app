...
function InvoiceInfo({onDelete}) {
  ...
  return (
    <div>
      {
        invoice ? 
        <motion.div key='invoice-info' initial={{x:0}} animate={{x:0}} exit={{ x: '200%'}} transition={{duration: 0.5}}
         className='dark:bg-[#141625] mx-auto duration-300 min-h-screen bg-[#f8f8fb] py-[34px] px-2 md:px-8 lg:px-12 max-w-3xl lg:py-[72px]'>
          <div>
            <button onClick={() => navigate(-1)} className='flex items-center space-x-4  group  dark:text-white font-thin'>
              ...
              <p className='group-hover:opacity-85'>Go BAck</p>
            </button>
            ...
            <div className='mt-8 rounded-lg w-full flex items-center justify-between px-6 py-6 bg-white dark:bg-[#1e2139]'>
              ...
              <div className='md:block hidden'>
                <button onClick={() => setIsEditOpen(true)} 
                  className='text-[#7e88c3] text-center dark:bg-[#252945] hover:opacity-80 
                   bg-slate-100 p-3 px-7 rounded-full'>
                    Edit
                </button>

                <button onClick={() => setIsDeleteModalOpen(true)} className='ml-3 text-center  text-white
                 bg-red-500 hover:opacity-80 p-3 px-7 rounded-full'>
                  Delete
                </button>
                {
                  invoice.status === 'pending' && 
                  <button onClick={onMakePaidClick} className='ml-3 text-center  text-white bg-[#7c5dfa]
                   hover:opacity-80 p-3 px-7 rounded-full' > 
                   Mark as Paid
                  </button>
                }
              </div>
